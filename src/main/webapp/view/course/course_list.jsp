    ...
    <script charset="UTF-8">
        ...
        function delCourse(courseId) {
            $.confirm('确认要删除吗？', function (isConfirm) {
                if (isConfirm) {
                    // $.alert('确认');
                    window.location.href = "<%=path%>/course/Delete?courseId=" + courseId;
                } else {
                    $.alert('取消');
                }
            });
...
<div class="row alert alert-info" style="margin:5px 0px 5px 0px; padding:3px;">
    <form class="form-inline" action="<%=path%>/course/search">
        <div class="form-group">
            <input type="text" name="key" class="form-control" id="activename" placeholder="请输入关键字">
        </div>
        <input type="submit" class="btn btn-danger" value="查询"/>
        <a class="btn btn-success" href="<%=path%>/view/course/course_add.jsp">增加</a>
        <span class="glyphicon glyphicon-trash del" aria-hidden="true" id="deleteButton">批量删除</span>
    </form>
</div>
<div class="row" style="padding:15px; padding-top:0px; ">
    <table class="table  table-condensed table-striped">
        ...
        <tr>
        <td><input id="check1" type="checkbox" value="1" name="allSelect"></td>
        ...
        <td>${course.department.departmentName}</td>
            ...
            <td>
                <a href="<%=path%>/course/getOne?courseId=${course.courseId}">
                    <span class="glyphicon glyphicon-edit" aria-hidden="true">修改</span>
                </a>
                <a href="javascript:delCourse(${course.courseId})">
        ...
        <tr align="center">
            <td colspan="5">
                <c:if test="${info.pageNum!=1}">
                    <a href="<%=path%>/course/listAll?page=1">首页</a>
                    <a href="<%=path%>/course/listAll?page=${info.prePage}">上一页</a>
                </c:if>
                <c:if test="${info.pageNum==1}">
                    首页
                    上一页
                ...
                <c:forEach items="${info.navigatepageNums}" var="num">
                    ...
                    <c:if test="${num != info.pageNum}">
                        <a href="<%=path%>/course/listAll?page=${num}">${num}</a>
                    </c:if>
                </c:forEach>
                <c:if test="${info.pageNum!=info.pages}">
                    <a href="<%=path%>/course/listAll?page=${info.nextPage}">下一页</a>
                    <a href="<%=path%>/course/listAll?page=${info.pages}">末页</a>
                </c:if>
                <c:if test="${info.pageNum==info.pages}">
                    下一页
                    末页
